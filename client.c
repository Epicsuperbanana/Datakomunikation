...
int main(int argc, char *argv[]) {
  ...
  while(1) {
    ...
    if(strncmp(messageString,"quit\n",messageLength) != 0){
      ...
      /*--LAB 2 PART 2: Recieve and print the server reply to our transmission--*/      
    
    int maximumLength = 100;
    char buff[maximumLength];
    int reply = recv(sock, buff, maximumLength, 0);
    if(reply == 0) {
	    printf("Recieving socket is closed");
    }
    else if (reply < 0){
	    printf("Could not read reply");
    }
    else{
